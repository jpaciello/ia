...
public class RLAgent {
    ...
    private double qRate = 0.1; //default qRate
    ...
    private void jugar(int jugador) { //elitista (max value)
    ...
    private void jugarRandom(int jugador) {//diversificación
    ...
    public void jugarVsRandom() {
        ...
        do {
            if (turno == jugador) {
                q = Math.random();
                if (q <= qRate || !this.entrenar) { //en train, juega basado en qrate (elitista o random)
                    jugar(jugador);                 //en validation, juega siempre elitista
                } else {
                    jugarRandom(jugador);
                }
            } else {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int trainingCount = 100000; //train
        int humanTrainingCount = 0; //train
        double totalGamesCount = 100000.0; //validation
        ...
        for (int q = 0; q < qRates.length; q++) {
            ...
            for (int k = 0; k < totalExperiments; k++) {
                ...
                ag.setAlpha(0.7); //suponemos que el humano juega bien (explotación)
                for (int i = 0; i < ag.getN(); i++) {
                    ag.reset(true);
                    //ag.updateAlpha(i);
                    ag.jugarVsHumano();
